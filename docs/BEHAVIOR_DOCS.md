...
| Listing video / CSV inventory | [LISTING_VIDEO_CSV_INVENTORY.md](LISTING_VIDEO_CSV_INVENTORY.md) | All inventory is CSV-listed; use Trading API only (GetItem read, ReviseFixedPriceItem for add/remove video via VideoDetails / DeletedField). Inventory API 404s for CSV SKUs. |
