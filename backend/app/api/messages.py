...
# Set when POST /sync?full=true arrives while another sync holds the lock; the running sync
# checks this after each committed page / phase and exits early so the full backfill can run.
_sync_interrupt_event = asyncio.Event()


async def _wait_until_lock_free(timeout: float = 300.0) -> None:
    """Wait until no task holds `_sync_lock` (poll; lock is not directly awaitable)."""
...
def _message_sort_key(m: Message) -> datetime:
    return m.ebay_created_at or datetime.min.replace(tzinfo=None)


def _response_media_type(raw_type: Any) -> str:
...
def _update_existing_message_media(existing: Message, media_list: List[dict]) -> None:
    if media_list:
        existing.media = media_list


def _build_message_media_items_for_response(
    m: Message,
    stored_blobs_by_key: Mapping[tuple[str, int], MessageMediaBlob],
) -> Optional[List[MessageMediaItem]]:
    out: List[MessageMediaItem] = []
    if m.media:
        for i, x in enumerate(m.media):
            url = (x.get("mediaUrl") or x.get("mediaURL") or "").strip() if isinstance(x, dict) else None
            if (m.message_id, i) in stored_blobs_by_key:
                url = _media_url_for_response(m.message_id, i)
            if isinstance(x, dict):
                out.append(
                    MessageMediaItem(
                        mediaName=x.get("mediaName") or "",
                        mediaType=_response_media_type(x.get("mediaType") or x.get("type")),
                        mediaUrl=url,
                    )
                )
    else:
        blobs = sorted(
            (
                blob
                for (message_id, _media_index), blob in stored_blobs_by_key.items()
                if message_id == m.message_id
            ),
            key=lambda blob: blob.media_index,
        )
        for blob in blobs:
            out.append(
                MessageMediaItem(
                    mediaName=blob.media_name or "",
                    mediaType=_response_media_type(blob.media_type),
...
async def _get_thread_impl(
    ...
    msgs = sorted(thread.messages, key=_message_sort_key)
    ...
    stored_blobs_by_key = {
        (blob.message_id, blob.media_index): blob
    ...
    def to_message_response(m: Message) -> MessageResponse:
        media = _build_message_media_items_for_response(m, stored_blobs_by_key)
        try:
...
async def get_message_media(
    ...
    """Serve stored attachment bytes. Used when we have a local copy (e.g. after eBay purge)."""
    if media_index < 0:
        raise HTTPException(status_code=400, detail="Invalid media index")
    result = await db.execute(
        select(MessageMediaBlob).where(
            MessageMediaBlob.message_id == message_id,
            MessageMediaBlob.media_index == media_index,
        )
    )
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'inline; filename="{row.media_name or "attachment"}"'},
    )
...
async def send_reply(
    ...
    if not seller_username or seller_username.lower() == "seller":
        for m in sorted(thread.messages, key=_message_sort_key):
    ...
    if body.draft_content is not None and body.draft_content.strip():
        ...
        buyer_summary = None
        for m in sorted(thread.messages, key=_message_sort_key, reverse=True):
...
async def refresh_thread_messages(
    ...
    for m in msgs:
        ...
        if existing:
            ...
            _update_existing_message_media(existing, media_list)
...
async def _store_message_media_blobs(
    ...
    for idx, item in enumerate(media_list):
        ...
        try:
            ...
            logger.warning("Failed to fetch message media for storage: message_id=%s index=%s url=%s: %s", message_id, idx, url[:80], e)
            continue
        if not data:
            continue
    ...
    try:
        ...
        logger.warning("Failed to commit message media blobs for message_id=%s: %s", message_id, e)
        await db.rollback()
...
async def sync_messages(
    ...
    runs at a time; concurrent calls receive 503. A **full** sync (full=1) signals any
    running sync to stop after the current safe checkpoint (committed page), then runs.
    """
    global _sync_in_progress
    if _sync_lock.locked():
        if full:
            _sync_interrupt_event.set()
            logger.info("Messages sync: full backfill requested; signaling running sync to stop at next checkpoint")
            try:
                await _wait_until_lock_free(timeout=300.0)
            except asyncio.TimeoutError:
                raise HTTPException(
                    status_code=status.HTTP_504_GATEWAY_TIMEOUT,
                    detail="Current sync did not stop within 5 minutes; try again.",
                )
            _sync_interrupt_event.clear()
        else:
            raise HTTPException(
...
async def _sync_from_members_full(
    ...
    *,
    ...
    If interruptible is True, stops between pages when a full backfill is waiting to preempt an incremental run.
    """
    threads_added = 0
    messages_added = 0
    offset = 0
    while True:
        ...
        for mid, mlist in page_messages_with_media:
            await _store_message_media_blobs(db, mid, mlist)
        if interruptible and _sync_interrupt_event.is_set():
            logger.info("Messages sync: FROM_MEMBERS full interrupted after safe page commit (offset=%s)", offset)
            break
...
async def _do_sync_messages(db: AsyncSession, full_sync: bool = False):
    ...
    try:
        ...
        if full_sync:
            ...
            t, m = await _sync_from_members_full(
                db, access_token, limit, seller_username, interruptible=False
            )
            threads_synced += t
            messages_synced += m
        else:
            ...
            if need_periodic_full and not _sync_interrupt_event.is_set():
                ...
                messages_synced += m
                ran_full_sync = True
            elif need_periodic_full and _sync_interrupt_event.is_set():
                logger.info("Messages sync: skipping periodic full (full backfill waiting)")
        ...
        if not _sync_interrupt_event.is_set():
            ...
            try:
                while pages_fetched < max_pages:
                    ...
                    if not full_sync and _sync_interrupt_event.is_set():
                        logger.info("Messages sync: FROM_EBAY interrupted after safe page commit (offset=%s)", offset)
                        break
            ...
            logger.info("Messages sync: skipping FROM_EBAY (full backfill waiting)")
    ...
    interrupted = _sync_interrupt_event.is_set()
    ...
    sync_summary["interrupted"] = interrupted
